...
{-# OPTIONS_GHC -fno-warn-unused-binds -fno-warn-unused-imports -fcontext-stack=312 #-}
...
type GraphHopperDirectionsAPI
    =    "geocode" :> QueryParam "q" Text :> QueryParam "locale" Text :> QueryParam "limit" Int :> QueryParam "reverse" Bool :> QueryParam "point" Text :> QueryParam "provider" Text :> QueryParam "key" Text :> Verb 'GET 200 '[JSON] GHGeocodingResponse -- 'geocodeGet' route
    :<|> "isochrone" :> QueryParam "point" Text :> QueryParam "time_limit" Int :> QueryParam "distance_limit" Int :> QueryParam "vehicle" Text :> QueryParam "buckets" Int :> QueryParam "reverse_flow" Bool :> QueryParam "key" Text :> Verb 'GET 200 '[JSON] GHIsochroneResponse -- 'isochroneGet' route
    :<|> "matrix" :> QueryParam "point" (QueryList 'MultiParamArray (Text)) :> QueryParam "from_point" Text :> QueryParam "to_point" Text :> QueryParam "out_array" (QueryList 'MultiParamArray (Text)) :> QueryParam "vehicle" Text :> QueryParam "key" Text :> Verb 'GET 200 '[JSON] GHMatrixResponse -- 'matrixGet' route
    :<|> "matrix" :> QueryParam "key" Text :> ReqBody '[JSON] GHMatrixRequest :> Verb 'POST 200 '[JSON] GHMatrixResponse -- 'matrixPost' route
...
data GraphHopperDirectionsBackend m = GraphHopperDirectionsBackend {
    geocodeGet :: Maybe Text -> Maybe Text -> Maybe Int -> Maybe Bool -> Maybe Text -> Maybe Text -> Maybe Text -> m GHGeocodingResponse{- ^ This endpoint provides forward and reverse geocoding. For more details, review the official documentation at: https://graphhopper.com/api/1/docs/geocoding/  -},
    isochroneGet :: Maybe Text -> Maybe Int -> Maybe Int -> Maybe Text -> Maybe Int -> Maybe Bool -> Maybe Text -> m GHIsochroneResponse{- ^ The GraphHopper Isochrone API allows calculating an isochrone of a locations means to calculate 'a line connecting points at which a vehicle arrives at the same time,' see [Wikipedia](http://en.wikipedia.org/wiki/Isochrone_map). It is also called **reachability** or **walkability**.  -},
    matrixGet :: Maybe [Text] -> Maybe Text -> Maybe Text -> Maybe [Text] -> Maybe Text -> Maybe Text -> m GHMatrixResponse{- ^ The Matrix API is part of the GraphHopper Directions API and with this API you can calculate many-to-many distances, times or routes a lot more efficient than calling the Routing API multiple times. In the Routing API we support multiple points, so called 'via points', which results in one route being calculated. The Matrix API results in NxM routes or more precise NxM weights, distances or times being calculated but is a lot faster compared to NxM single requests. The most simple example is a tourist trying to decide which pizza is close to him instead of using beeline distance she can calculate a 1x4 matrix. Or a delivery service in the need of often big NxN matrices to solve vehicle routing problems. E.g. the GraphHopper Route Optimization API uses the Matrix API under the hood to achieve this.  -},
    matrixPost :: Maybe Text -> GHMatrixRequest -> m GHMatrixResponse{- ^ The GET request has an URL length limitation, which hurts for many locations per request. In those cases use a HTTP POST request with JSON data as input. The only parameter in the URL will be the key which stays in the URL. Both request scenarios are identically except that all singular parameter names are named as their plural for a POST request.  -},
    ...
    routeGet :: Maybe [Text] -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe Bool -> Maybe Bool -> Maybe Bool -> Maybe [Text] -> Maybe Bool -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe Int -> Maybe Int -> Maybe Bool -> Maybe Int -> Maybe Integer -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Text -> m GHRouteResponse{- ^ The GraphHopper Routing API allows to calculate route and implement navigation via the turn instructions -}
  }

newtype GraphHopperDirectionsClient a = GraphHopperDirectionsClient { runClient :: Manager -> BaseUrl -> ExceptT ServantError IO a }
    deriving Functor

instance Applicative GraphHopperDirectionsClient where
    pure x = GraphHopperDirectionsClient (\_ _ -> pure x)
    (GraphHopperDirectionsClient f) <*> (GraphHopperDirectionsClient x) = GraphHopperDirectionsClient (\manager url -> f manager url <*> x manager url)

instance Monad GraphHopperDirectionsClient where
    (GraphHopperDirectionsClient a) >>= f = GraphHopperDirectionsClient (\manager url -> do
        value <- a manager url
...
createGraphHopperDirectionsClient = GraphHopperDirectionsBackend{..}
  where
    ((coerce -> geocodeGet) :<|>
     ...
     (coerce -> getSolution) :<|>
     (coerce -> postVrp) :<|>
     (coerce -> routeGet)) = client (Proxy :: Proxy GraphHopperDirectionsAPI)
