...
type GraphHopperDirectionsAPI
    ...
    :<|> "isochrone" :> QueryParam "point" Text :> QueryParam "time_limit" Int :> QueryParam "vehicle" Text :> QueryParam "buckets" Int :> QueryParam "reverse_flow" Bool :> QueryParam "key" Text :> Verb 'GET 200 '[JSON] GHIsochroneResponse -- 'isochroneGet' route
...
data GraphHopperDirectionsBackend m = GraphHopperDirectionsBackend {
    ...
    isochroneGet :: Maybe Text -> Maybe Int -> Maybe Text -> Maybe Int -> Maybe Bool -> Maybe Text -> m GHIsochroneResponse{- ^ The GraphHopper Isochrone API allows calculating an isochrone of a locations means to calculate 'a line connecting points at which a vehicle arrives at the same time,' see [Wikipedia](http://en.wikipedia.org/wiki/Isochrone_map). It is also called **reachability** or **walkability**.  -},
