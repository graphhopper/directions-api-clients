...
# **isochroneGet**
> \Swagger\Client\Model\GHIsochroneResponse isochroneGet($point, $key, $time_limit, $vehicle, $buckets, $reverse_flow)
...
$time_limit = 600; // int | Specify which time the vehicle should travel. In seconds. The maximum depends on the subscribed package.
...
try {
    $result = $api_instance->isochroneGet($point, $key, $time_limit, $vehicle, $buckets, $reverse_flow);
    print_r($result);
} catch (Exception $e) {
