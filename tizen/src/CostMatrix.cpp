...
CostMatrix::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("CostMatrix_data")) {
			jsonToValue(&data, node, "CostMatrix_data", "CostMatrix_data");
		} else {
			
			CostMatrix_data* obj = static_cast<CostMatrix_data*> (&data);
...
CostMatrix::toJson()
{
	...
	if (isprimitive("CostMatrix_data")) {
		CostMatrix_data obj = getData();
		node = converttoJson(&obj, "CostMatrix_data", "");
	}
	else {
		
		CostMatrix_data obj = static_cast<CostMatrix_data> (getData());
		GError *mygerror;
		mygerror = NULL;
...
void
CostMatrix::setData(CostMatrix_data  data)
{
	this->data = data;
}
