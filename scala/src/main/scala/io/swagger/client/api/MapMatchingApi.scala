...
class MapMatchingApi(
  ...
  val helper = new MapMatchingApiAsyncHelper(client, config)
   ...
   */
  def postGPX(): Option[RouteResponse] = {
    val await = Try(Await.result(postGPXAsync(), Duration.Inf))
    await match {
   ...
   */
  def postGPXAsync(): Future[RouteResponse] = {
      helper.postGPX()
  }

}

class MapMatchingApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {

  def postGPX()(implicit reader: ClientResponseReader[RouteResponse]): Future[RouteResponse] = {
    // create path and map variables
    val path = (addFmt("/match"))
