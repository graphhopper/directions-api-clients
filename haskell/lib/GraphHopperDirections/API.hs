...
data GraphHopperDirectionsBackend m = GraphHopperDirectionsBackend
  ...
  , routeGet :: Maybe [Text] -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe Bool -> Maybe Bool -> Maybe Bool -> Maybe [Text] -> Maybe Bool -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe Int -> Maybe Int -> Maybe Bool -> Maybe Int -> Maybe Integer -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> m RouteResponse{- ^ The GraphHopper Routing API allows to calculate route and implement navigation via the turn instructions -}
