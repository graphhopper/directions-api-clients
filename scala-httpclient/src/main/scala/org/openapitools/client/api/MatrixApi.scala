...
class MatrixApi(
   ...
   * @return MatrixResponse
   */
  def matrixPost(key: String, matrixRequest: Option[MatrixRequest] = None): Option[MatrixResponse] = {
    val await = Try(Await.result(matrixPostAsync(key, matrixRequest), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * @return Future(MatrixResponse)
   */
  def matrixPostAsync(key: String, matrixRequest: Option[MatrixRequest] = None): Future[MatrixResponse] = {
      helper.matrixPost(key, matrixRequest)
  }
...
class MatrixApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def matrixPost(key: String,
    matrixRequest: Option[MatrixRequest] = None
    ...
    val resFuture = client.submit("POST", path, queryParams.toMap, headerParams.toMap, writer.write(matrixRequest))
    resFuture flatMap { resp =>
      process(reader.read(resp))
    }
  }
