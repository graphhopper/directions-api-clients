...
Solution::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("std::string")) {
			jsonToValue(&unassigned, node, "std::string", "");
		} else {
			
			std::string* obj = static_cast<std::string*> (&unassigned);
...
Solution::toJson()
{
	...
	if (isprimitive("std::string")) {
		std::string obj = getUnassigned();
		node = converttoJson(&obj, "std::string", "");
	}
	else {
		
		std::string obj = static_cast<std::string> (getUnassigned());
		GError *mygerror;
		mygerror = NULL;
...
void
Solution::setUnassigned(std::string  unassigned)
{
