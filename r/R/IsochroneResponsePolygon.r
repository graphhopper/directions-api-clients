...
IsochroneResponsePolygon <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(IsochroneResponsePolygonJson) {
      ...
      self$`properties` <- objectObject$fromJSON(jsonlite::toJSON(IsochroneResponsePolygonObject$properties, auto_unbox = TRUE))
      self$`type` <- IsochroneResponsePolygonObject$`type`
      objectObject <- object$new()
      self$`geometry` <- objectObject$fromJSON(jsonlite::toJSON(IsochroneResponsePolygonObject$geometry, auto_unbox = TRUE))
    }
  )
)
