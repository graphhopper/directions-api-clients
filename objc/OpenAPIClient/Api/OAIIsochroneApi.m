...
@interface OAIIsochroneApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIIsochroneApi

NSString* kOAIIsochroneApiErrorDomain = @"OAIIsochroneApiErrorDomain";
NSInteger kOAIIsochroneApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) isochroneGetWithPoint: (NSString*) point
    ...
    if (point == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIIsochroneApiErrorDomain code:kOAIIsochroneApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIIsochroneResponse*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIIsochroneResponse*)data, error);
                                }
                            }];
}
