...
import io.swagger.client.models.ResponseAddress

/**
 * 
 * @param type type of activity
 * @param id Id referring to the underlying service or shipment, i.e. the shipment or service this activity belongs to
 * @param locationId Id that refers to address
 * @param address 
 * @param arrTime Arrival time at this activity in seconds. If type is `start`, this is not available (since it makes no sense to have `arr_time` at start). However, `end_time` is available and actually means \\\"departure time\\\" at start location. It is important to note that `arr_time` does not necessarily mean \\\"start of underlying activity\\\", it solely means arrival time at activity location. If this activity has no time windows and if there are no further preparation times, `arr_time` is equal to activity start time.
 * @param endTime End time of and thus departure time at this activity. If type is `end`, this is not available (since it makes no sense to have an `end_time` at end) `end_time` at each activity is equal to the departure time at the activity location.
 * @param endDateTime End date time with offset like this 1970-01-01T01:00+01:00. If you do not use time-dependent optimization, this is `null`.
 * @param arrDateTime Arrival date time with offset like this 1970-01-01T01:00+01:00. If you do not use time-dependent optimization, this is `null`.
 * @param waitingTime Waiting time at this activity in seconds. A waiting time can occur if the activity has at least one time window. If `arr_time` < `time_window.earliest` a waiting time of `time_window_earliest` - `arr_time` occurs.
 * @param preparationTime preparation time at this activity in seconds
 * @param distance cumulated distance from start to this activity in m
 * @param drivingTime driving time of driver in seconds
 * @param loadBefore Array with size/capacity dimensions before this activity
 * @param loadAfter Array with size/capacity dimensions after this activity
 */
data class Activity (
    ...
    val address: ResponseAddress? = null,
    ...
    val arrDateTime: java.time.LocalDateTime? = null,
    ...
    val drivingTime: kotlin.Long? = null,
    /* Array with size/capacity dimensions before this activity */
    val loadBefore: kotlin.Array<kotlin.Int>? = null,
    /* Array with size/capacity dimensions after this activity */
    val loadAfter: kotlin.Array<kotlin.Int>? = null
) {
    /**
    * type of activity
